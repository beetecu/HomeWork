...
/**
 * Testing the Wu Manber method from command line 
 * 
 * ex
 * java org.stringmatching.tests.Test_Arg_WM -p run,SAU -f FlashPlayerInstallManager.log
 * patterns:  run SAU
 * doing matching using an input file: FlashPlayerInstallManager.log
 * [ Test id: WM-----execution time: 1ms -----From position: 0 ]
 ...
 *
 */
public class Test_Arg_WM {

	static final List<byte[]> _byte_test_patterns  = new ArrayList<byte[]>();
	static String test_id = "WM";
	private static WM_Pattern wmPattern;
	private static WM_Matcher ma;
	...
	public static void main(String[] args) throws Exception {
		...
		SimpleJSAP jsap = new SimpleJSAP( 
				"Home Work", 
				"Wu Manber Pattern matcher method",
				new Parameter[] {
		...
		if ((patterns.length > 0) || (loadFilepath.length() > 0)){
			...
			if (patterns.length > 0){
				...
				try{
				    ...
				    wmPattern = new WM_Pattern (loadFilepath);
		...
		try{
			if (saveFilepath.length() > 0){
				...
				wmPattern.savePatterns(saveFilepath);
			}
		}catch(Exception e){
